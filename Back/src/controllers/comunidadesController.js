...
class ComunidadeController {
    ...
    static listarComunidades = (req, res) => {
        ...
        .populate('jogo')
        .populate({ path: 'membros', select: 'nome & foto',  populate: { path : 'jogos' } })
        .populate('topicos')
    ...
    static listarComunidadePorId = (req, res) => {
        ...
        .exec((err, comunidade) => {
            if(err) {
                res.status(400).send({message: `${err.message} - Id do comunidade não localizado`})
            } else {
    ...
    static cadastrarComunidade = (req, res) => {
        ...
        comunidade.save((err) => {
            if(err) {
                res.status(500).send({message: `${err.message} - falha ao cadastrar comunidade`})
            } else {
